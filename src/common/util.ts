 ...
 * @returns 深度，最小1
 */
export const getDeep = <T extends AnyObjType>(
  arr: Array<T>,
  childKey: string = "children"
): number => {
  const res = arr.map(obj => {
    if (
      Object.hasOwnProperty.call(obj, childKey) &&
      Array.isArray(obj[childKey])
    ) {
 ...
 * @returns 拉平后的数组
 */
export const arrayFlat = <T extends AnyObjType>(
  arr: Array<T>,
  childKey: string = "children",
 ...
 * @returns 该深度的元素集合
 */
export const getItemsByDeep = <T extends AnyObjType>(
  arr: Array<T>,
  childKey: string = "children",
  deep: number = Infinity
): Array<T> => {
